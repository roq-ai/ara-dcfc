...
import { getContractors } from 'apiSdk/contractors';
...
function DashboardPage() {
  ...
  const contractorStat = useSWR<PaginatedInterface<ContractorInterface>>(
    () => `/contractors`,
    () => getContractors({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Contractors',
      result: contractorStat?.data?.totalCount,
      isLoading: contractorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
